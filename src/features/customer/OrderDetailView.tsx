import { CalendarDays, ChevronLeft, Mail, MapPin, Package, ReceiptText, Truck } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="space-y-6">
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-neutral-200 text-left text-xs uppercase tracking-[0.12em] text-neutral-400">
                    <th className="px-0 py-3 font-semibold">Product</th>
                ...
                <tbody>
                  {order.items.map((item) => (
                    <tr key={item.id} className="border-b border-neutral-100 last:border-b-0">
                      <td className="py-4 pr-4">
                        <div className="flex items-center gap-4">
                          <img
                            ...
                            className="h-16 w-16 rounded-2xl border border-neutral-200 bg-neutral-50 object-cover"
                          />
                          <div>
                      ...
                      <td className="px-4 py-4 text-neutral-600">{item.variantName || '—'}</td>
                      <td className="px-4 py-4 font-mono text-xs text-neutral-500">{item.sku || '—'}</td>
                      <td className="px-4 py-4 text-right font-medium text-neutral-900">{item.quantity}</td>
          ...
          <SectionCard icon={<ReceiptText size={18} />} title="Payment Summary">
            <DetailRow label="Payment Status" value={<OrderPaymentStatusBadge status={order.payment?.status || 'UNKNOWN'} />} />
            ...
            <DetailRow label="Provider Reference" value={order.payment?.providerReference || order.paymentReference || '—'} />
            <DetailRow label="Settlement Time" value={formatDateTime(order.payment?.settlementTime)} />
            <DetailRow label="Subtotal" value={formatCurrency(order.subtotal, order.currency)} />
            <DetailRow label="Shipping" value={formatCurrency(order.shippingCost, order.currency)} />
            <DetailRow label="Discount" value={formatCurrency(order.discount, order.currency)} />
            <DetailRow label="Tax" value={formatCurrency(order.tax, order.currency)} />
            <DetailRow label="Grand Total" value={formatCurrency(order.grandTotal, order.currency)} />
          </SectionCard>
        ...
        <div className="space-y-6">
          ...
          <SectionCard icon={<CalendarDays size={18} />} title="Order Summary">
            <DetailRow label="Order Number" value={order.orderNumber} />
            <DetailRow label="Created Date" value={formatDateTime(order.createdAt)} />
            <DetailRow label="Fulfillment Status" value={<OrderStatusBadge status={order.fulfillmentStatusLabel || order.fulfillmentStatus} />} />
            <DetailRow label="Total Products" value={String(order.items.length)} />
            <DetailRow label="Grand Total" value={formatCurrency(order.grandTotal, order.currency)} />
          </SectionCard>
        </div>
      </div>
