...
export function ImpactPage() {
  ...
  const loadPage = useCallback(
    async (
      ...
      try {
        const result: ImpactListPayload = await impactService.getImpactList(
          status,
          sort,
          offset,
          limit,
        ...
        if (offset === 0) {
          ...
          setState((current) => {
            ...
            return {
              ...
              items: [
                ...current.items,
                ...result.items.filter((item) => !existingIds.has(item.slug)),
              ],
  ...
  return (
    <div className="min-h-screen bg-white font-['SF-Pro-Display',_sans-serif] text-neutral-900">
      <main>
        ...
        <section
          className={`mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 ${settings.title ? "mt-14 sm:mt-20" : "pt-24 sm:pt-28"}`}
        >
