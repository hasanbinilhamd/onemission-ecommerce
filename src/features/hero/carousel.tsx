...
export const HERO_GRADIENTS = HERO_THEMES.map((item) => createHeroGradient(item.accentColor));
...
export function mapHeroItems(items: readonly WebsiteHeroItem[]): HeroCarouselItem[] {
  return [...items]
...
export function resolveHeroMediaSource(item: HeroCarouselItem, isMobile: boolean): string {
...
export function getHeroPoster(item: HeroCarouselItem, isMobile: boolean): string {
  return item.poster || item.blurMedia || resolveHeroMediaSource(item, isMobile);
}

export function getHeroBlurMedia(item: HeroCarouselItem, isMobile: boolean): string {
...
export function getRole(index: number, activeIndex: number, totalItems: number): Role {
  if (totalItems <= 1) {
...
export function useHeroGesture({
  ...
  const resolveGestureNavigation = useCallback((shouldNavigate: boolean) => {
    ...
    const gestureSurfaceWidth = surfaceRef.current?.clientWidth || window.innerWidth || 1;
